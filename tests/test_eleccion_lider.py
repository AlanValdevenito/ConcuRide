...
def start_leader_with_id(id):
    subprocess.Popen([f"cd server && cargo run {id} true false"], shell=True, stdout=subprocess.PIPE)

def start_replica_with_id(id):
    subprocess.Popen([f"cd server && cargo run {id} false false"], shell=True, stdout=subprocess.PIPE)
